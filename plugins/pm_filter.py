...
async def pm_text(bot, message):
    ...
    await message.reply_text(
         text=f"<b>ʜᴇʏ {user} 😍 ,\n\nʏᴏᴜ ᴄᴀɴ'ᴛ ɢᴇᴛ ᴍᴏᴠɪᴇs ꜰʀᴏᴍ ʜᴇʀᴇ. ʀᴇǫᴜᴇsᴛ ɪᴛ ɪɴ ᴏᴜʀ <a href=https://t.me/+lULqM33QsWMxNThl>Topic Group</a> ᴏʀ ᴄʟɪᴄᴋ ʀᴇǫᴜᴇsᴛ ʜᴇʀᴇ ʙᴜᴛᴛᴏɴ ʙᴇʟᴏᴡ ⟱</b>",   
         reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📝 ʀᴇǫᴜᴇsᴛ ʜᴇʀᴇ ", url=f"https://t.me/+lULqM33QsWMxNThl")]])
    )
    ...
    try:
        if settings['max_btn']:
            ...
            if n_offset == 0:
                ...
                btn.append([InlineKeyboardButton("𝐏𝐚𝐠𝐞", callback_data="pages"), InlineKeyboardButton(f"{math.ceil(int(offset)/10)+1} / {math.ceil(total/10)}", callback_data="pages"), InlineKeyboardButton("𝐍𝐞𝐱𝐭◆⫸", callback_data=f"next_{req}_{key}_{n_offset}")])
            else:
                btn.append(
                    [
            ...
            if n_offset == 0:
                ...
                btn.append([InlineKeyboardButton("𝐏𝐚𝐠𝐞", callback_data="pages"), InlineKeyboardButton(f"{math.ceil(int(offset)/int(MAX_B_TN))+1} / {math.ceil(total/int(MAX_B_TN))}", callback_data="pages"), InlineKeyboardButton("𝐍𝐞𝐱𝐭◆⫸", callback_data=f"next_{req}_{key}_{n_offset}")])
            else:
                btn.append(
                    [
                        InlineKeyboardButton("⫷◆𝐁𝐚𝐜𝐤", callback_data=f"next_{req}_{key}_{off_set}"),
...
async def filter_languages_cb_handler(client: Client, query: CallbackQuery):
    ...
    if offset != "":
        try:
            if settings['max_btn']:
                btn.append(
                    [InlineKeyboardButton("𝐏𝐚𝐠𝐞", callback_data="pages"), InlineKeyboardButton(text=f"1/{math.ceil(int(total_results)/10)}",callback_data="pages"), InlineKeyboardButton(text="𝐍𝐞𝐱𝐭◆⫸",callback_data=f"next_{req}_{key}_{offset}")]
                )
    
            else:
                btn.append(
                    [InlineKeyboardButton("𝐏𝐚𝐠𝐞", callback_data="pages"), InlineKeyboardButton(text=f"1/{math.ceil(int(total_results)/int(MAX_B_TN))}",callback_data="pages"), InlineKeyboardButton(text="𝐍𝐞𝐱𝐭◆⫸",callback_data=f"next_{req}_{key}_{offset}")]
                )
        except KeyError:
            await save_group_settings(query.message.chat.id, 'max_btn', True)
            btn.append(
        ...
        btn.append(
            [InlineKeyboardButton(text="𝐍𝐨 𝐦𝐨𝐫𝐞 𝐏𝐚𝐠𝐞 𝐚𝐯𝐚𝐢𝐥𝐚𝐛𝐥𝐞",callback_data="pages")]
        )
...
async def qualities_cb_handler(client: Client, query: CallbackQuery):
    ...
    btn.insert(
        ...
        [
            InlineKeyboardButton(
                text="⇊ 𝐒𝐞𝐥𝐞𝐜𝐭 𝐘𝐨𝐮𝐫 𝐐𝐮𝐚𝐥𝐢𝐭𝐲 ⇊", callback_data="ident"
            )
        ],
    )
    req = query.from_user.id
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        btn = [[
                InlineKeyboardButton("𝐔𝐧𝐚𝐯𝐚𝐥𝐚𝐛𝐥𝐞", callback_data=f"unavailable#{from_user}"),
                InlineKeyboardButton("𝐔𝐩𝐥𝐨𝐚𝐝𝐞𝐝", callback_data=f"uploaded#{from_user}")
             ],[
                InlineKeyboardButton("𝐀𝐥𝐫𝐞𝐚𝐝𝐲 𝐀𝐯𝐚𝐥𝐚𝐛𝐥𝐞", callback_data=f"already_available#{from_user}")
              ]]
        btn2 = [[
        ...
        ident, from_user = query.data.split("#")
        btn = [[
                InlineKeyboardButton("⚠️ 𝐔𝐧𝐚𝐯𝐚𝐥𝐚𝐛𝐥𝐞 ⚠️", callback_data=f"unalert#{from_user}")
              ]]
        btn2 = [[
        ...
        btn2 = [[
                 ...
                 InlineKeyboardButton("𝐑𝐞𝐪𝐮𝐬𝐭 𝐆𝐫𝐨𝐮𝐩 𝐋𝐢𝐧𝐤", url="https://WD_Topic_Group")
               ]]
        if query.from_user.id in ADMINS:
            user = await client.get_users(from_user)
            reply_markup = InlineKeyboardMarkup(btn)
        ...
        ident, from_user = query.data.split("#")
        btn = [[
                InlineKeyboardButton("𝐀𝐥𝐫𝐞𝐚𝐝𝐲 𝐀𝐯𝐚𝐥𝐚𝐛𝐥𝐞", callback_data=f"alalert#{from_user}")
              ]]
        btn2 = [[
                 InlineKeyboardButton('𝐉𝐨𝐢𝐧 𝐂𝐡𝐚𝐧𝐧𝐞𝐥', url=link.invite_link),
                 InlineKeyboardButton("𝐕𝐢𝐞𝐰 𝐒𝐭𝐚𝐭𝐮𝐬", url=f"{query.message.link}")
               ],[
                 InlineKeyboardButton("𝐑𝐞𝐪𝐮𝐬𝐭 𝐆𝐫𝐨𝐮𝐩 𝐋𝐢𝐧𝐤", url="https://telegram.me/WD_Topic_Group")
               ]]
        if query.from_user.id in ADMINS:
            user = await client.get_users(from_user)
            reply_markup = InlineKeyboardMarkup(btn)
    ...
    elif OpleechData.startswith("generate_stream_link"):
        _, file_id = OpleechData.split(":")
        try:
            user_id = query.from_user.id
            ...
            Opleech_stream = f"{URL}watch/{str(log_msg.id)}/{quote_plus(get_name(log_msg))}?hash={get_hash(log_msg)}"
            Opleech_download = f"{URL}{str(log_msg.id)}/{quote_plus(get_name(log_msg))}?hash={get_hash(log_msg)}"
            ...
            return
    # don't change anything without contacting me @Opleech
        ...
        buttons = [[
                    ...
                    InlineKeyboardButton('◆ 𝐌𝐨𝐯𝐢𝐞 𝐆𝐫𝐨𝐮𝐩 ◆', url="https://t.me/WD_Topic_Group")
                ],[
        ...
        buttons = [[
            InlineKeyboardButton('𝐌𝐚𝐧𝐮𝐚𝐥 𝐅𝐥𝐢𝐭𝐞𝐫', callback_data='manuelfilter'),
            InlineKeyboardButton('𝐀𝐮𝐭𝐨 𝐅𝐥𝐢𝐭𝐞𝐫', callback_data='autofilter')
        ],[
        ...
        buttons = [[
             ...
             InlineKeyboardButton('♻️ 𝐂𝐨𝐧𝐧𝐞𝐜𝐭𝐢𝐨𝐧𝐬 ♻️', callback_data='coct'), 
        ...
        buttons = [[
            ...
            InlineKeyboardButton('𝐒𝐨𝐮𝐫𝐜𝐞 𝐂𝐨𝐝𝐞', callback_data='source')
             ...
             InlineKeyboardButton('𝐑𝐞𝐧𝐝𝐞𝐫𝐢𝐧𝐠 𝐢𝐧𝐟𝐨', callback_data='rendering_info')
        ],[            
        ...
        await query.message.edit_text(
            ...
            btn = [[
                    InlineKeyboardButton("⫷◆𝐁𝐚𝐜𝐤", callback_data="help"),
                    InlineKeyboardButton("𝐂𝐨𝐧𝐭𝐚𝐜𝐭", url="telegram.me/Farooq_is_KING")
                  ]]
            await client.edit_message_media(
                query.message.chat.id, 
                query.message.id, 
...
async def auto_filter(client, msg, spoll=False):
    ...
    if imdb:
        ...
        if not settings["button"]:
            cap+="<b>\n\n<u>🍿 𝐘𝐨𝐮𝐫 𝐌𝐨𝐯𝐢𝐞 𝐅𝐢𝐥𝐞 ⟱</u></b>\n"
            for file in files:
                cap += f"<b>\n📁 <a href='https://telegram.me/{temp.U_NAME}?start=files_{file.file_id}'>[{get_size(file.file_size)}] {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@') and not x.startswith('www.'), file.file_name.split()))}\n</a></b>"
    else:
        if settings["button"]:
            cap = f"<b>𝐓𝐡𝐞 𝐑𝐞𝐬𝐮𝐥𝐭𝐬 𝐅𝐨𝐫 ☞ {search}\n\n𝐑𝐞𝐪𝐮𝐬𝐭𝐞𝐝 𝐁𝐲 ☞ {message.from_user.mention}\n\n𝐑𝐞𝐬𝐮𝐥𝐭 𝐒𝐡𝐨𝐰 𝐢𝐧 ☞ {remaining_seconds} 𝐒𝐞𝐜𝐨𝐧𝐝𝐬\n\n𝐏𝐨𝐰𝐞𝐫𝐞𝐝 𝐁𝐲 ☞ : {message.chat.title} \n\n⚠️ 𝐀𝐟𝐭𝐞𝐫 5 𝐌𝐢𝐧𝐮𝐭𝐞𝐬 𝐓𝐡𝐢𝐬 𝐌𝐞𝐬𝐬𝐚𝐠𝐞 𝐖𝐢𝐥𝐥 𝐁𝐞 𝐀𝐮𝐭𝐨𝐦𝐚𝐭𝐢𝐜𝐚𝐥𝐥𝐲 𝐃𝐞𝐥𝐞𝐭𝐞𝐝  🗑\n\n</b>"
        else:
            cap = f"<b>𝐓𝐡𝐞 𝐑𝐞𝐬𝐮𝐥𝐭𝐬 𝐅𝐨𝐫 ☞ {search}\n\n𝐑𝐞𝐪𝐮𝐬𝐭𝐞𝐝 𝐁𝐲 ☞ {message.from_user.mention}\n\n𝐑𝐞𝐬𝐮𝐥𝐭 𝐒𝐡𝐨𝐰 𝐢𝐧 ☞ {remaining_seconds} 𝐒𝐞𝐜𝐨𝐧𝐝𝐬\n\n𝐏𝐨𝐰𝐞𝐫𝐞𝐝 𝐁𝐲 ☞ : {message.chat.title} \n\n⚠️ 𝐀𝐟𝐭𝐞𝐫 5 𝐌𝐢𝐧𝐮𝐭𝐞𝐬 𝐓𝐡𝐢𝐬 𝐌𝐞𝐬𝐬𝐚𝐠𝐞 𝐖𝐢𝐥𝐥 𝐁𝐞 𝐀𝐮𝐭𝐨𝐦𝐚𝐭𝐢𝐜𝐚𝐥𝐥𝐲 𝐃𝐞𝐥𝐞𝐭𝐞𝐝  🗑\n\n</b>"
            cap+="<b><u>🍿 𝐘𝐨𝐮𝐫 𝐌𝐨𝐯𝐢𝐞 𝐅𝐢𝐥𝐞 ⟱</u></b>\n\n"
            for file in files:
